    ...
    if (@available(iOS 15.0, *)) {
        if (GCController.controllers.count == 0 && _virtualController != nil) {
            [_virtualController connectWithReplyHandler:^(NSError * _Nullable error) {
                if (error) {
                    LOG_ERROR_FMT("Failed to connect virtual controller: %@", error);
                } else {
                    LOG_INFO("Virtual controller connected successfully");
                }
            }];
        }
    }
    ...
    if (@available(iOS 15.0, *)) {
        if (_virtualController != nil) {
            ...
            if (hasPhysicalController) {
                LOG_INFO("Physical controller connected. Disconnecting virtual controller");
                [_virtualController disconnect];
            }
        }
    }
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    LOG_INFO_FMT("Metal device created: %@", [self.device name]);
    ...
    if (@available(iOS 13.0, *)) {
        ...
        if (style == UIUserInterfaceStyleDark) {
            ...
            LOG_INFO("Using dark theme");
        } else {
            ImGui::StyleColorsLight();
            LOG_INFO("Using light theme");
        }
    } else {
        ImGui::StyleColorsDark();
    }
    ...
    if (fontPath) {
        ...
        LOG_INFO_FMT("Loaded SF Mono font from: %@", fontPath);
    } else {
        LOG_WARNING("Warning: Could not find SFMonoRegular.ttf, using default font");
    }
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification {
    ...
    LOG_INFO("Application initialized and window displayed");
